...
def bubbleSort_status(tmp):
    for i in range(len(tmp)-1,-1,-1):
        for j in range(i):
            if tmp[j]>tmp[j+1]:
                tmp[j],tmp[j+1]=tmp[j+1],tmp[j]

def sortingStatus(status,sorted_count):
    tmp=[]
    sorted_status=[]
    sorted_status_count=[]
    for i in status:
        tmp.append(i)
    bubbleSort_status(tmp)
    for i in tmp:
...
def counting_sort(sorted_log,log,sorted_count):
...
def main():
    ...
    readfile(status,log)
    sortingStatus(status,sorted_count)
    counting_sort(sorted_log,log,sorted_count)
    writeLog(sorted_log)
    return sorted_count
